...
from torchvision.transforms.transforms import Optional
import random

class RollingShutter(torch.nn.Module):
    def __init__(self, std: float = 1.0):
        super().__init__()
        self.std = float(np.random.uniform(0, std))
...
class Kernel(object):
    ...
    def _createKernel(self, save_to: Optional[Path]=None, show: bool=False):
        """[summary]
    ...
    def displayKernel(self, save_to: Optional[Path]=None, show: bool=True):
        """[summary]
        Finds a kernel (psf) of given intensity.
        [description]
    ...
    def applyTo(self, image, keep_image_dim: bool = False) -> Image.Image:
        """[summary]
        ...
        def applyToPIL(image: Image.Image, keep_image_dim: bool = False) -> Image.Image:
            """[summary]
...
class MotionBlur(torch.nn.Module):
    def __init__(self):
        super().__init__()
        self.kernels = [
            Kernel(size=(16, 16), intensity = 0.5) for _ in range(16)
        ]

    def forward(self, img: Image.Image) -> Image.Image:
        kernel = random.choice(self.kernels)
        return kernel.applyTo(img, True)
